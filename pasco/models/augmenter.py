...
class Augmenter(object):
    def __init__(
        ...
        flip=True,
        scale_range=0.1,
        max_translation=np.array([1.0, 1.0, 0.5]),
    ):
    ...
    def merge(self, in_feat):
        ...
        in_feat_dense = in_feat.dense(
            min_coordinate=torch.IntTensor([*min_coordinate]).to(in_feat.device)
        )[0]

        in_feat_dense = torch.cat(
            [in_feat_dense[t] for t in range(in_feat_dense.shape[0])], dim=0
        ).unsqueeze(0)
